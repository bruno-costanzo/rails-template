...
- `app/views/documents/index.html.erb` — the search form carries a scan button under the `native-only` class, so it exists only in the app: the shell opens the barcode scanner, fills the query field with the code and submits the search. The scanner sheet's copy lives under `charco_mobile.scanner` in both locale files.
