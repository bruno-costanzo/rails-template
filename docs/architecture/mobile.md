...
- `app/models/user.rb`, `app/models/application_push_device.rb`, `app/models/application_push_notification.rb`, `app/jobs/application_push_notification_job.rb`, `config/push.yml` — push notifications through `action_push_native`. The layout renders `native_push_tag` for signed-in people; the app asks for permission once and posts its token to `/native/push/devices`, which the gem resolves to `Current.user` through `config/initializers/charco_mobile.rb` and stores on the person's `push_devices`, moving it there when another person registered the same token on that phone. When the identity signal disappears on sign-out, the app deletes its token with `DELETE /native/push/devices/:token`, which the gem answers outside `ApplicationController` because the session is already gone; `ApplicationPushDevice` is the default class it deletes from. `config/push.yml` still carries placeholders for the Apple team id, the APNs topic and the Firebase project id; delivery needs the credentials it points at in `bin/rails credentials:edit`.
...
A push token belongs to the phone, not to the person: the gem reassigns a token to whoever signs in and deletes it on sign-out, so a notification for one account never lands on a phone someone else now holds. The platform is whatever `ApplicationPushDevice` validates, `apple` or `google`, and anything else is refused with 422.
