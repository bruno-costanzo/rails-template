...
- **Ticket lifecycle** — `Feedback` gains `status` (`open`/`resolved`) and `resolved_at`. A minimal `/admin/feedbacks` panel behind credentials-based HTTP basic auth (same lookup pattern as Solid Errors, but denying access entirely when no credentials are configured) lists open tickets with their context and photos and marks them resolved; resolving sends a thank-you email to the reporter via Action Mailer. Ticket state lives in the app; the GitHub issue is an unsynced working mirror.
