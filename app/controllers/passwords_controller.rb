class PasswordsController < ApplicationController
  ...
  rate_limit to: 10, within: 3.minutes, only: :create, with: -> { redirect_to new_password_path, alert: t("passwords.rate_limit") }
  ...
  def update
    if @user.update(params.permit(:password, :password_confirmation))
      ...
      redirect_to edit_password_path(params[:token]), alert: t("passwords.update.alert")
    end
  end
